pub mod windows_printer; // Declare the submodule

use std::{cell::RefCell, ffi::c_void, rc::Rc};

pub use self::windows_printer::WindowsPrinter;
use escpos::driver::Driver; // Corrected path
                ...
                if StartDocPrinterW(printer_handle, 1, &document_info) == 0 {
                    ...
                    if !StartPagePrinter(printer_handle).as_bool() {
                        ...
                        .as_bool()
                        {
                            ...
                            error = Some(PrinterError::Io(
                                "Failed to write all bytes to printer".to_owned(),
                            ));
                            eprintln!("Error: {:?}", error);
                        }
                    }
